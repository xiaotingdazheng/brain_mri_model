...
pathRegisteredFloatingImage = fullfile(resultsFolder,[filename '.registered_to_' refBrainNum '.nii.gz']); %path of registered floating image
aff = fullfile(resultsFolder, [filename,'.aff']); %deformation of first registration
pathTransformation = fullfile(resultsFolder, [filename '.registered_to_' refBrainNum '.cpp.nii.gz']); %modify name of the saved aff file
...
if isequal(labelPriorType, 'delta function')
    ...
    pathRegisteredFloatingLabels = fullfile(resultsFolder, [filename,'.registered_to_',refBrainNum,'.nii.gz']); % path of registered segmentation map
    ...
    [~,filename,~] = fileparts(temp_floSegm);
    pathRegisteredFloatingHippoLabels = fullfile(resultsFolder, [filename,'.registered_to_',refBrainNum,'.nii.gz']); % path of registered segmentation map
    % apply registration to segmentation map
