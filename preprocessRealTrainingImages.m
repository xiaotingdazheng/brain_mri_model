...
for channel=1:nChannel
    ...
    for i=1:length(structPathsTrainingImages)
        ...
        if rescale
            ...
            if channel == 1
                pathMaskedTrainingImage = mask(pathRescaledTrainingImage, pathTrainingLabels, pathFolderFloatingImages, channel, 0, 0, ...
                    floBrainNum, pathTempImFolder, freeSurferHome, recompute, 1, 5);
            else
            ...
            if channel == 1
                pathMaskedTrainingImage = mask(pathTrainingImage, pathTrainingLabels, pathFolderFloatingImages, channel, 0, 0, floBrainNum,...
                    pathTempImFolder, freeSurferHome, recompute, 1, 5);
            else
        ...
        if channel == 1 && (recompute || ~exist(cellPathsNewImages{i,channel}, 'file') || ~exist(cellPathsNewLabels{i,channel}, 'file'))
            ...
            [~,~] = system(cmd1);
            [~,~] = system(cmd2);
            mask(cellPathsNewImages{i,channel}, cellPathsNewLabels{i,channel}, cellPathsNewImages{i,channel}, 0, NaN, 0, floBrainNum, pathTempImFolder, freeSurferHome, 1, 0, 5);
        end
        ...
        if multiChannel && channel > 1
            ...
            mask(cellPathsNewImages{i,channel}, cellPathsNewLabels{i,1}, cellPathsNewImages{i,channel}, 0, NaN, 0, floBrainNum, pathTempImFolder, freeSurferHome, 1, 0, 5);
        end
